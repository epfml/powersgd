...
description = """
Here, we evaluate whether reuse brings us closer to a power iteration with more steps
""".strip()
...
for rank in [1]:
    for reuse in [True, False]:
        if reuse:
            ...
            reducer = f"Rank {rank} w/o reuse (EF)"
    ...
    # Higher quality baseline with more power iteration steps
    reducer = f"HQ Rank {rank} w/o reuse (EF)"
    name = f"{reducer}"
    if mongo.job.count_documents({"job": name, "experiment": experiment}) > 0:
        # We have this one already
        continue
